...
public class InfoActivity extends AppCompatActivity {
            ...
            switch (opt){
                case "건물":{
                    ...
                    switch (info){
                        ...
                        case "호수":{Q = "SELECT 호실, 분류 FROM Building_table WHERE 건물이름 = \""+res+"\""; break;}
                        // 이 건물에서 진행되는 모든 과목들
                        case "과목":{Q = "SELECT 교과목명, 전공, 분반 FROM Time_table WHERE 건물1 = \""+res+"\" OR 건물2 = \""+res+"\""; break;}
                    }
                    break;
                }
                case "호수":{
                    ...
                    switch (info){
                        ...
                        // 이 방에서 진행되는 과목
                        case "과목":{
                            Q = "SELECT 교과목명, 전공, 분반 FROM Time_table WHERE 강의실1 = "+room_num+" OR 강의실2 = "+room_num; break;
                        }
                    }
                    break;
                }
                case "교수":{
                    switch (info){
                        case "과목":{Q = "SELECT 교과목명, 전공, 분반 FROM Time_table WHERE 담당교수 = \""+res+"\"";break;}
                    }
                    break;
                }
                case "과목":{
    ...
    public LinearLayout make_linear_layout_lec(List<String> res, int id){
        ...
        if(room2 != "none"){ lay.addView(tv_room2); }
        return lay;
    }
